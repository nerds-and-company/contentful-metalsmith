 ...
 */
function getCommonContentForSpace (entries, options, locale = null) {
  if (!options.common) {
    return entries
  }
  ...
  for (let id in options.common) {
    const common = Object.assign({},
      options.common[id],
      (locale && !options.common[id].locale ? { locale } : {})
    )
    commonQueries.push(client.getEntries(util.getEntriesQuery(common, options.filterTransforms)))
    commonIds.push(id)
  }
...
function processFile (file, options) {
  ...
  return client.getEntries(query)
    ...
    .then(entries => getCommonContentForSpace(entries, options, file.contentful.locale || null))
}
